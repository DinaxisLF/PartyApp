...
import { SafeAreaView, Text, View, TouchableOpacity, ScrollView, Image, TextInput, StyleSheet } from "react-native";
...
export default function PortfolioScreen() {
  ...
  const [activeTab, setActiveTab] = useState<"portafolio" | "preview">("portafolio");
    ...
    >
      <SafeAreaView className="h-full">
        ...
        <View className="flex-row self-center mt-2">
          <TouchableOpacity
            className={`h-[28px] w-[158px] rounded-[4px] ${activeTab === "portafolio" ? "bg-[#7E3AF2]" : "bg-[#374151]"}`}
            onPress={() => setActiveTab("portafolio")}
          >
            <Text className="font-biryani color-white text-[13px] text-center pt-[3px]">Portafolio</Text>
          </TouchableOpacity>

          <TouchableOpacity
            className={`h-[28px] w-[158px] rounded-[4px] ${activeTab === "preview" ? "bg-[#7E3AF2]" : "bg-[#374151]"}`}
            onPress={() => setActiveTab("preview")}
          >
            <Text className="font-biryani color-white text-[13px] text-center pt-[3px]">Preview</Text>
          </TouchableOpacity>
        </View>

        {activeTab === "portafolio" ? (
          <ScrollView className="self-center mt-5">
            <View className="bg-slate-800 rounded-xl shadow-md w-96 h-auto justify-center items-center mb-5">
              <Text style={styles.sectionTitle}>Fotos</Text>
              <View className="flex-row flex-wrap gap-2 justify-center mb-5">
                {images.map((uri, index) => (
                  <View key={index} className="w-36 h-36 bg-white rounded-sm justify-center items-center">
                    <Image
                      source={{ uri }}
            ...
            <View className="bg-slate-800 rounded-xl shadow-md w-96 h-auto justify-center items-center mb-5 p-2">
              ...
              <View style={styles.row}>
                ...
                >
                  <FontAwesome name="cloud-upload" size={24} color="black" />
                  <Text className="text-black text-center font-biryani-semibold text-xs mt-1">Sube un video</Text>
                </TouchableOpacity>
              </View>
            </View>
            ...
            <View className="bg-slate-800 rounded-xl shadow-md w-96 h-auto justify-center items-center mb-5 p-2">
              ...
              <View className="flex-col gap-2 mb-5 w-full px-5">
                <Text className="text-white font-biryani-semibold text-lg mt-1">Facebook</Text>
                ...
                <Text className="text-white font-biryani-semibold text-lg mt-1">Instagram</Text>
                <TouchableOpacity className="bg-white rounded-md p-2">
                  <TextInput
                    style={styles.textInput}
                ...
                <Text className="text-white font-biryani-semibold text-lg mt-1">TikTok</Text>
                <TouchableOpacity className="bg-white rounded-md p-2">
                  <TextInput
                    style={styles.textInput}
                ...
                <View className="flex-row">
                  {previewVideos.map((uri, index) => (
                    <Video
                      key={index}
                      source={{ uri }}
                      style={{ width: 240, height: 135, borderRadius: 10, marginRight: 16 }}
                      useNativeControls
                      resizeMode={ResizeMode.CONTAIN}
                    />
                  ))}
